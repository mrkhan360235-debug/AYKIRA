...
const ADMIN_PASSWORD =
  process.env.AYKIRA_ADMIN_PASSWORD || 'AYKIRA@1234';

function defaultData() {
  return {
    products: [],
    settings: {
      eyebrow: 'ETHNIC • FESTIVE • OCCASION WEAR',
      title: 'Little looks. Big moments.',
      lead: 'Beautiful ethnic and occasion wear, selected with love for every celebration.',
      collection: 'More designs coming soon.',
      festive: 'Styles for Eid, Diwali, weddings, parties and special occasions.',
      size: 'Our collection is available across sizes 18–38.',
      pricing: {
        '18-22': 749,
        '24-32': 999,
        '34-38': 1199
...
function loadData() {
  try {
    ...
    return {
      ...
      settings: x.settings || defaultData().settings
    };
  } catch (_) {
    return defaultData();
  }
...
async function handleApi(
    ...
    try {
      ...
      return json(
          ...
          : 400,
        {
          error:
            err.code === 'PAYLOAD_TOO_LARGE'
              ? 'Catalogue payload is too large.'
              : 'Invalid JSON.'
        }
      );
    }
  ...
  if (
    ...
    try {
      ...
      return json(res, 500, {
        error: 'Unable to save catalogue.'
      });
    }
  }
  ...
  if (
    ...
    if (
      ...
      return json(res, 400, {
        error:
          'Amount must be an integer of at least 100 paise.'
      });
    }
    ...
    try {
      ...
      if (err.statusCode === 401) {
        return json(res, 401, {
          error:
            'Razorpay authentication failed. Check the API credentials.'
        });
      }
  ...
  if (
    ...
    if (oi >= 0) {
      orders[oi].payment_status =
        'paid';

      orders[oi].status =
        'New';

      orders[oi].razorpay_payment_id =
        paymentId;
...
server.listen(
  ...
  () =>
    console.log(
      `AYKIRA server running at http://localhost:${PORT}`
    )
);
