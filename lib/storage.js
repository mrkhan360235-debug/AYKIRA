...
class PostgresStore {
 ...
 async image(name,bytes){if(!this.env.BLOB_READ_WRITE_TOKEN)throw error(503,'Photo storage is not configured.');const {put}=require('@vercel/blob');const ext=path.extname(name);const blob=await put(`${this.schema}/products/${name}`,bytes,{access:'public',token:this.env.BLOB_READ_WRITE_TOKEN,contentType:{'.png':'image/png','.jpg':'image/jpeg','.webp':'image/webp','.gif':'image/gif'}[ext],addRandomSuffix:false,allowOverwrite:true,cacheControlMaxAge:31536000});return blob.url;}
}
