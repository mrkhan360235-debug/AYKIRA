...
class FileStore {
 ...
 async releaseRejectedOrder(id){const s=this.read();if(s.orders[id]&&!s.orders[id].razorpay_order_id&&s.orders[id].payment_status==='pending'){delete s.orders[id];this.write(s);}}
...
class PostgresStore {
 ...
 async migrate(){
  ...
  if(this.env.VERCEL_ENV==='production'){
   const [source]=await this.query("SELECT to_regclass('aykira_v23_preview.catalog') AS source");
   if(source?.source)await this.query(`INSERT INTO ${s}.catalog (id,data,revision) SELECT id,data,revision FROM aykira_v23_preview.catalog WHERE id=1 ON CONFLICT(id) DO NOTHING`);
  }
 ...
 async releaseRejectedOrder(id){await this.query(`DELETE FROM ${this.schema}.orders WHERE id=$1 AND razorpay_order_id IS NULL AND data->>'payment_status'='pending'`,[id]);}
